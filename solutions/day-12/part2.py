...
with open("input.txt", "r") as fio:
    ...
    for m,y in enumerate(inverted_heatmap):
        for n,x in enumerate(y):
            if inverted_heatmap[m][n] == 'a':
                result, visited = get_solution(inverted_heatmap, [n,m], 'E')
                
                if result >= 0:
                    all_results.append(result)
                    print(f"starting at {[0,20]} took {result} steps")
